...
$('#mpchile').on('change',function(event) {
  
   $('#mpagoFinalChile').val($('#mpchile').val()); 
    console.log($('#mpagoFinalChile').val());
}); 


$('#pais_aux').on('change',function(event) {
var pais=$('#pais_aux').val();
	if(pais!='null'){
		if(pais==='Chile(PS-38)'){
		/*	
		...
		*/
		...
		$('#mpagoExt').removeClass("d").addClass("d-none"); 
		$('#mpagoChile').removeClass("d-none").addClass("d"); 
		...
		//$('#mpagoExt').removeClass("d-none").addClass("d"); 
		//$('#mpagoChile').removeClass("d").addClass("d-none"); 
		}
	}
 });
//Datos de pagador
...
//container.setAttribute("class", ".hidden-xs-down" );card card-body  .hidden-xs-down  mx-auto
...
alert ('No olvide subir sus Antecedentes');
	...
	$("#form_pre").validate({
      ...
      messages:{
        ...
        "pais_aux"        :{required:"Selecciona tu país de Residencia"},		
		...
		"mpago"           :{required:"Favor de Selecciona algun medio de Pago"}     
      },
      errorElement: "span",
